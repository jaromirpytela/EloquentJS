...
// 1) Find script by assigning a full Unicode character.
...
// 2) Count the characters that belong to each script (the same itemProperty).

function countBy(items, groupFunction){
  let output=[];
  for (let i of items){
    let iProperty=groupFunction(i);
    let iFound=output.findIndex(c=>c.iProperty==iProperty);
    if (iFound==-1){
      output.push({iProperty, count: 1})
    } else {
      output[iFound].count++
    }
  }
...
function dominantDirection(text) {
  let counted = countBy(text, char => { //get script from char
    let script = characterScript(char.codePointAt(0)); 
    return script ? script.direction : "none"; 
  }).filter(({iProperty}) => iProperty != "none"); 

// Count total amount of characters that belong to a script
return counted.reduce((a, b) => a.count > b.count ? a : b).iProperty;
}
