...
// The characterScript and countBy functions are useful here. Find script by assigning a full Unicode character.

function characterScript(code) {
    for (let script of SCRIPTS) {
        if (script.ranges.some(([a, b]) => code >= a && code < b)) {
            return script;
        }
    }
    return null;
...
// Count the characters that belong to each script and return object with iProperty (grouping is false or true) and count property.

function countBy(array, group) {
    let output=[];
    for(let i of array){
        let grouping=group(i);
        let iFound=output.findIndex(c=>c.grouping===grouping);
        if(iFound===-1){
            output.push({grouping, count: 1})
        } else {
            output[iFound].count++;
        }
    }
...
/*  COMPUTE DOMINANT DIRECTION
  > Count the chars by script name with characterScript.  
  > Fall back to "none" for chars that aren’t part of any script.
  > Drop the entry for "none" from the resulting array.
  > Count total amount of chars that belong to a script
*/
